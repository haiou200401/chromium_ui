...
#if 1 // gloam:
  const string16 url_string = UTF8ToUTF16(url.host());
#else
...
#if 1 //gloam:
  string16 url_domain = UTF8ToUTF16(url.host());
#else
...
SortedDisplayURL::SortedDisplayURL(const GURL& url,
                                   const std::string& languages) {
#if 1 //gloam:
  sort_host_ = UTF8ToUTF16(url.host());

  const string16 www(ASCIIToUTF16("www."));
  string16 host_minus_www = StartsWith(sort_host_, www, true) ? sort_host_.substr(www.length()) : sort_host_;

  // ......
  display_url_ = host_minus_www;
#else
